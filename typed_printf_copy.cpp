...
template<typename R, char c, typename X, R (*cont)(X)>
auto print_const(X x) {
    cout << c;
    return cont(x);
}

template<typename R, char c, typename X, R (*cont)(void)>
auto print_const() {
    cout << c;
    return cont();
}
...
constexpr auto d_type_printf() {
    if constexpr(format[i] == '%' && format[i + 1] == 'd') {
        constexpr auto cont = d_type_printf<format, i + 2>();
        ...
        constexpr auto cont = d_type_printf<format, i + 1>();
...
#define def_typed_printf(f, str) constexpr static const char str_fmt##f[] = str; auto f = d_type_printf<str_fmt##f,0>();

#define ABC "F47if9Zxw9jXEfQ68VtGA"
#define BAC "J6Y6Xc88UrUtpDkK3iF8p"
#define CAB "7s4y2gzdG8Ao2BMgv6aiJ"
